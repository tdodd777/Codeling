...
### 2026-05-12 — OTEL receivers fail in packaged app (issue #3, v0.1.6)

- **Symptom:** packaged 0.1.5 launched but `sessions` / `otel_events` stayed empty. HTTP receiver threw `"XMLHttpRequest is not defined"`; gRPC threw `"Cannot read properties of null (reading 'readFileSync')"`. Both were swallowed by `try/catch` in `startReceivers`, so the Settings UI still reported "Listening."
- **Cause #1 — Vite bundled `protobufjs` / `@grpc/grpc-js` / `@grpc/proto-loader`.** Only `electron` + `better-sqlite3` were in `external`. `protobufjs` has a `"browser"` field Rollup picks by default → XHR loader. `@grpc/grpc-js` does dynamic native-binding `require`s that don't survive bundling. Fix: add the three to `external` in `vite.main.config.ts`. The stale "node_modules aren't shipped" comment was wrong — `copyProductionDeps` in `forge.config.ts` already copies the full prod dep closure into the build, so externalizing pure-JS deps is safe.
- **Cause #2 — Settings "Listening" hint read the persisted flag only.** `Settings.tsx` switched on `telemetry.enabled` (the meta-table flag) and ignored `telemetry.running` (the live `isReceiversRunning()` value), even though the IPC handler at `ipc.ts:176` returns both. Fix: three-state hint — Paused / Listening / "Failed to start — check app logs and retry the toggle."
- **What we did NOT change.** The lifecycle still swallows start errors so a half-up state is possible (HTTP up, gRPC down or vice versa). The Settings hint now surfaces "Failed to start" when *either* receiver is down (since `isReceiversRunning()` is `httpServer || grpcServer`), which is approximately right but conflates partial-up with fully-down. Worth tightening to track per-receiver state if the bundling fix doesn't fully solve the in-the-wild reports.

### 2026-05-12 — First published release (v0.1.3) + packaging fixes

- **Forge `outDir` moved out of the workspace** to `~/.codeling-build/`. Cause: VS Code's file watcher held a handle on `out/resources/app.asar` between runs, recurring `EBUSY: resource busy or locked, unlink` on the next `npm run make`. Killing all `Code.exe` processes wasn't an option since Claude Code runs *inside* VS Code in this dev setup — it'd terminate the agent. Writing artifacts outside the workspace sidesteps the watcher entirely. Override via `FORGE_OUT_DIR` env var if needed.
- **`.vscode/settings.json` ships `files.watcherExclude` for `out/` + `.vite/`** as belt-and-suspenders against the same lock if someone reverts the `outDir` override. Whitelisted via `!.vscode/settings.json` in `.gitignore` so the exclusion travels with the repo.
- **npm `prepublishOnly` / `postpublish` swap `package.json#main` to the CLI entry around publish.** npm force-includes the file at `main` regardless of the `files` whitelist — pointing main at `.vite/build/main.js` (required by Electron + Forge) bloated the npm tarball from ~10 kB to ~400 kB. Lifecycle hooks call `scripts/npm-main-swap.mjs --swap` before pack and `--restore` after, leaving `package.json` byte-identical for dev/Forge work. Backup file `.npm-main-backup` guards against half-finished swaps; gitignored. `prepublishOnly` is intentional over `prepack` — the latter also fires on `npm install` from a git URL, the former is publish-only.
- **GitHub Release v0.1.3 is the first public artifact set.** Setup.exe (Squirrel/Windows), .nupkg, and RELEASES manifest live at `https://github.com/tdodd777/Codeling/releases/tag/v0.1.3`. `update.electronjs.org` now resolves against it. macOS/Linux makers are wired in `forge.config.ts` but only Windows artifacts were built today — running `npm run release` on a Mac would produce the DMG; on a Linux box the .deb/.rpm.
- **npm `codeling@0.1.6` is `latest`** (updated 2026-05-12 with the OTel-bundling fix from #3). `0.1.0` and `0.1.1` deprecated with "Broken installer (blank panel + missing native deps). Use 0.1.3 or later." `0.1.2` was version-bumped in `package.json` (commit `6d6baf9`) but never reached npm — the lock chain blocked the publish, so there's nothing to deprecate. Published roster: `0.1.0`, `0.1.1`, `0.1.3`, `0.1.4`, `0.1.5`, `0.1.6` — no `0.1.2`.
- **Verify path: `npx -y codeling@latest install --skip-otel --skip-hook` from a temp dir.** Resolves `codeling@0.1.3` against the npm registry, downloads the 117 MB Setup.exe from the GitHub Release via the GitHub API, spawns the Squirrel installer detached, exits clean. `--skip-otel --skip-hook` keeps the user's env + `~/.claude/settings.json` untouched on verification runs.
...
- ~~**Tray icon doesn't refresh when species changes mid-session.**~~ Resolved 2026-05-11 by `14dbabe` — option (a): 5s polling drift check compares current pet.species against the last-rendered species and calls `startTrayAnimation` if they diverge. Catches direct-DB writes that bypass the event channel.
- ~~**Sprite scanner alias-collision (last-write-wins on `manifest.animations[alias]`).**~~ Resolved 2026-05-11 by `5175927` — option (a) "fuller-anim-wins": the scanner only overwrites an existing alias entry when the candidate has more total frames. Mimic's three Idle folders now resolve to the multi-frame Idle animation instead of one of the single-frame static poses.
...
- ~~**Sprite manifest fetched 3× per panel mount.**~~ Resolved 2026-05-11 by `7b2e8f0` — Home owns the manifest fetch; PetSprite takes it as a prop. StrictMode double-mount in dev still produces 2 fetches but that's dev-only and one source.
