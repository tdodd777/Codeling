...
- **npm `codeling@0.1.3` is `latest`.** `0.1.0` and `0.1.1` deprecated with "Broken installer (blank panel + missing native deps). Use 0.1.3 or later." `0.1.2` was version-bumped in `package.json` (commit `6d6baf9`) but never reached npm — the lock chain blocked the publish, so there's nothing to deprecate. The roster of *published* versions is therefore `0.1.0`, `0.1.1`, `0.1.3` — no `0.1.2`.
